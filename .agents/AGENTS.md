...
Key decisions:
- **Go is the definitive scaffolder; Python is a second engine, not a legacy one.** Both implement `onboard-team` and `add-service` against the same `catalog.yaml`. The point of keeping two is that it makes the catalog a *falsifiable* contract: run both with the same inputs and `diff -r` the trees. They currently agree on every generated file except one whitespace line in `catalog-info.yaml` (tracked as Phase 1a in `2-idp-scaffolder/python/TODO.md`). If a change makes them disagree, either the engines drifted or the catalog is under-specified — both are findings.
