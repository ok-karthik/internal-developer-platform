...
5. **System Grouping for Blast-Radius Isolation:** The `systems/<system>/` grouping is maintained to provide a clean Git directory anchor for ArgoCD ApplicationSet auto-discovery, and to create clear boundaries for Terraform blast-radius isolation. (Note: While `catalog-info.yaml` stubs are included for Backstage service-catalog awareness, Backstage is not the architectural spine of this layout).
...
9. **Taxonomy (Team vs. Tenant):** We explicitly use `<team>` as the ownership boundary (e.g., `3-tenant-workloads/<team>`) because teams are the customers of our product. We restrict the word "tenant" to the isolation control plane (`tenant-foundation/`) rather than using it as the owner name, demonstrating precise usage of infrastructure vs. business vocabulary.

10. **Data-Driven Scaffolder (Catalog Destinations):** The CLI avoids hardcoded output paths. Instead, a `destinations:` ABI mapping block in `1-platform-catalog/catalog.yaml` defines the precise target directories for teams, systems, runtimes, and capabilities. The Go scaffolder dynamically maps these (e.g. replacing `{team}`, `{system}`) before writing, ensuring the templates and the CLI are decoupled.
11. **Plan-then-Write CLI Architecture:** The Go scaffolder operates on a `Plan-then-Write` architecture (using `fs.FS` and an in-memory `Plan` map). This structurally prevents half-scaffolded outputs if an error occurs mid-generation, strictly aligns `--dry-run` with actual execution, and allows unit tests to run entirely in-memory using golden files without touching the local disk.
