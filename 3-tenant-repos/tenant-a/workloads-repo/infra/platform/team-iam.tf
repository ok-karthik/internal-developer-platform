...
# Python engine allocated into 3-tenant-repos/cloud_vpcs_allocated.yaml.
...
# once a real EKS cluster's OIDC issuer exists (the cloud foundation lives in
# enterprise-aws-infrastructure; this repo only has the k3d harness under 1-cloud-foundation/local/). See "IRSA / Pod Identity is the missing fourth wall of
...
module "aws-iam" {
  source    = "git::https://github.com/ok-karthik/enterprise-aws-infrastructure.git//iac-modules-repo/identity/workload-iam?ref=workload-iam-v2.0.0"
  team_name = "tenant-a"
  app_name  = "shared"
}
